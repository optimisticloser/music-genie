...
} from "@/components/shared/PlaylistFilters";
...
export default function FavoritesPage() {
  ...
  const missingSupabaseMessage = t("missingSupabase.description");
  ...
  const loadPlaylists = useCallback(
    async (page: number) => {
      try {
        ...
        setPlaylistsError(t("errors.load"));
      } finally {
        setLoadingPlaylists(false);
      }
    },
    [filters, pagination.limit, locale, t]
  );
  ...
  useEffect(() => {
    async function getUser() {
      if (!supabaseClient) {
        console.warn(
          "Skipping favorites Supabase calls because the client is unavailable."
        );
  ...
  const handleToggleFavorite = async (
    ...
    if (success) {
      ...
      setPagination((prev) => ({
        ...
        totalPages: Math.max(Math.ceil((prev.total - 1) / prev.limit), 0),
      }));
    }
  };
          ...
          <Card>
            <CardContent className="p-6">
              <h2 className="text-xl font-semibold text-gray-900 mb-2">
                {t("missingSupabase.title")}
              </h2>
              <p className="text-gray-600 text-sm">{missingSupabaseMessage}</p>
            </CardContent>
          </Card>
        </div>
      </ScrollArea>
        ...
        <div className="mb-8">
          {loadingPlaylists ? (
            ...
            <div className="text-center py-8">
              ...
              <Button onClick={() => loadPlaylists(1)}>{t("buttons.retry")}</Button>
            </div>
          ) : playlists.length === 0 ? (
                      ...
                      <div className="relative">
                        <div
                          className={`aspect-square bg-gradient-to-br ${
                            playlist.gradient || "from-gray-400 to-gray-600"
