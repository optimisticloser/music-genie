...
export default function HistoryPage() {
  ...
  const missingSupabaseMessage =
    "Supabase client is not configured. Set NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY to enable history features.";
  ...
  useEffect(() => {
    async function getUser() {
      if (!supabaseClient) {
        console.warn("Skipping history Supabase calls because the client is unavailable.");
        ...
        <div className="mb-8">
          {loadingPlaylists ? (
            ...
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-semibold text-gray-900 mb-2">Nenhuma playlist encontrada</h3>
              <p className="text-gray-600 mb-6">
                {filters.search || filters.favoritesOnly || filters.genre || filters.mood
                  ? "Tente ajustar os filtros para encontrar mais playlists."
                  : "Crie sua primeira playlist com IA para começar!"}
              </p>
              {!filters.search && !filters.favoritesOnly && !filters.genre && !filters.mood && (
                      ...
                      <div className="relative">
                        ...
                        <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-20 transition-all duration-200 rounded-t-lg flex items-center justify-center">
                          <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex gap-2">
                            ...
                            >
                              <Heart className={`w-4 h-4 ${playlist.is_favorite ? 'fill-red-500 text-red-500' : ''}`} />
                            </Button>
                          </div>
                        </div>
                      </div>
